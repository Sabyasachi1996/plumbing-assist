...
export const chatController = {
  ...
  async handleChat(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.log(`[CHAT] Image uploaded. Searching active calls for Session ID: ${sessionId}`);
      console.log(`[CHAT] Current active phone calls in memory:`, Array.from(activeCalls.keys()));
      ...
      if (activeSocket) {
        ...
        // If they are on a call, force the voice agent to speak the reply!
        activeSocket.ws.send(JSON.stringify({
          response_type: "agent_interrupt",          // <-- Tells Retell to interrupt!
          interrupt_id: Math.floor(Math.random() * 900000) + 100000, // <-- Random ID generated by us
          content: spokenContent,
          content_complete: true,
          no_interruption_allowed: false
        }));

        // Send a visual placeholder to the text chat widget
        res.status(200).json({ 
