...
export const voiceController = {
  createWebCall: asyncHandler(async (req: Request, res: Response) => {
    const agentId = "agent_fe1fe5390ab304f9fb93615a97"; 
    ...
    if (!webCallResponse || !webCallResponse.call_id) {
      throw new AppError("Failed to initialize call with Retell AI.", 502); 
    }
  ...
  handleStream(ws: WebSocket, req: IncomingMessage) {
    ...
    ws.on('message', async (data: Buffer) => {
      try {
        ...
        // When user stops speaking, trigger the Orchestrator
        if (event.interaction_type === 'response_required' && sessionId && companyId) {
          ...
          // 1. Fetch History & Filter old system prompts
          ...
          // 3. Let Orchestrator handle state, dynamic prompts, and tool calls
          ...
          // Instead of text matching, check if the tool router pushed the END_CHAT signal!
          const shouldHangUp = aiResponse.pendingSignals.some((signal: any) => signal.action === "END_CHAT");        
          
          // 5. Send voice payload back to Retell
