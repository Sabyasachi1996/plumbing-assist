...
import crypto from "crypto"; // Added for Soft Hold token generation
...
export const aiService = {
  ...
  async analyzeImage(base64Image: string, mimeType: string): Promise<string> {
    try {
      ...
      // FIX: Added to access the first choice array element
  ...
  async generateResponse(chatHistory: any[], companyId: string, sessionId: string, retryCount = 0): Promise<{ reply: string, updatedMessages: any[], pendingSignals: any[] }> {
    try {
      let pendingSignals: any[] = [];
      let state = await redisService.getSessionState(sessionId);
      
      // Filter out old system prompts so we don't duplicate them in the array
      let cleanHistory = chatHistory.filter(m => m.role !== "system");
      const lastMsg = cleanHistory[cleanHistory.length - 1]?.content || "";

      // =====================================================================
      // 1. THE BACKEND INTERCEPTORS (HARD STATE JUMPS)
      // =====================================================================
      
      // A. Image Uploaded -> Jump to Date Selection
      ...
      // B. Details Submitted -> SOFT HOLD -> Jump to Payment
      if (state.currentState === "STATE_6_CUSTOMER_DETAILS" && lastMsg.includes("[SYSTEM_DETAILS_SUBMITTED]")) {
        ...
        // SOFT HOLD LOGIC: Generate a temporary hold token, but DO NOT book the calendar yet!
        const tempToken = `HOLD_${crypto.randomBytes(3).toString("hex").toUpperCase()}`;
        
        // Push UI Signal and Transition State to Payment
        pendingSignals.push({ action: "SHOW_PAYMENT_MODAL", data: { amount: 70, trackingToken: tempToken } });
        
        // Save the details to Redis so we have them ready when they actually pay
        state = await redisService.transitionState(sessionId, "STATE_7_PAYMENT", { customerDetails, trackingToken: tempToken });
        
        // Command the AI
        cleanHistory[cleanHistory.length - 1].content = `[SYSTEM] The user submitted their details. The slot is on a soft hold. Command: Tell the user you have reserved the slot and ask them to complete the advance payment on their screen.`;
      }

      // C. Payment Successful -> HARD BOOKING -> Jump to Confirmation
      if (state.currentState === "STATE_7_PAYMENT" && lastMsg.includes("[SYSTEM_PAYMENT_SUCCESSFUL]")) {
        // NOW we reconstruct the booking args from our Redis memory state and book it!
      ...
      const messages = [{ role: "system", content: prompt }, ...cleanHistory];
      ...
      if (toolCalls && toolCalls.length > 0) {
        ...
        for(const toolCall of toolCalls){
          ...
          if (functionName === "recordIssue") {
            ...
            pendingSignals.push({ action: "SHOW_SLOT_PICKER", data: { slots: ["10:00 to 12:00", "12:00 to 14:00", "14:00 to 16:00", "16:00 to 18:00"] }});
      ...
      if (groqError?.code === "tool_use_failed" && retryCount < 2) {
        ...
        return this.generateResponse(chatHistory, companyId, sessionId, retryCount + 1);
      }
