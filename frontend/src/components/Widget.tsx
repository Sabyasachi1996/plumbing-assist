...
export default function Widget({ orgId, sessionId }: { orgId: string, sessionId: string }) {
  ...
  // Initialize WebSockets and Retell
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (payload.action === 'CHAT_REPLY') {
        ...
        setUiState({ type: 'SLOTS', data: payload.data.slots });
      } 
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {isOpen && (
        <div className="bg-white w-[350px] h-[550px] max-w-[calc(100vw-3rem)] max-h-[calc(100vh-8rem)] rounded-2xl shadow-2xl border border-slate-200 mb-4 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 relative">
          ...
          <div className="bg-blue-600 text-white p-4 flex justify-between items-center z-10 relative shadow-md">
            ...
            {/* Remove the switch button since mode is locked after selection */}
          </div>

          {/* ========================================================= */}
          {/* STATE 1: SELECTION GATEWAY                                */}
          {/* ========================================================= */}
          {mode === 'selection' && (
            <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-6 bg-slate-50">
              <div className="text-center space-y-2">
                <div className="w-16 h-16 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Bot size={32} />
                </div>
                <h3 className="font-bold text-xl text-slate-800">Need a plumber?</h3>
                <p className="text-sm text-slate-500">Choose how you would like to connect with our AI dispatch assistant.</p>
              </div>
              
              <div className="w-full space-y-3">
                <button 
                  onClick={() => setMode('text')} 
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-md transition flex items-center justify-center gap-2"
                >
                  <MessageSquare size={20} /> Start Text Chat
                </button>
                <button 
                  onClick={startVoiceCall} 
                  className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-4 rounded-xl shadow-md transition flex items-center justify-center gap-2"
                >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 bg-slate-50 space-y-8 relative">
              ...
              {uiState.type === 'ENDED' ? (
                 <div className="text-center space-y-3">
                    ...
                    <p className="text-slate-500 text-sm">Thank you for reaching out.</p>
                 </div>
              ) : (
                <>
                  <div className="relative flex items-center justify-center mt-8">
                    {/* Pulsing rings for active listening effect */}
                  ...
                  <button 
                    ...
                    className="mt-auto bg-red-500 hover:bg-red-600 text-white font-bold py-3 px-8 rounded-full shadow-lg transition flex items-center gap-2"
                  >
          ...
          {mode === 'text' && (
            <>
              ...
              {/* THE "ACTION DRAWER": Sits below chat, above input */}
              {uiState.type !== 'NONE' && uiState.type !== 'ENDED' && (
                <div className="bg-white border-t border-slate-200 p-4 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-10 animate-in slide-in-from-bottom-2">
                  ...
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button 
                      ...
                      className="text-slate-400 hover:text-slate-700 bg-slate-100 hover:bg-slate-200 p-1 rounded-full transition"
                    >
                      <X size={14} />
                    </button>
                  </div>
                  ...
                  {uiState.type === 'UPLOAD' && (
                    <div className="text-center">
                      <button onClick={() => fileInputRef.current?.click()} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 py-3 rounded-xl font-medium transition flex items-center justify-center gap-2 border border-slate-200">
                  ...
                  {uiState.type === 'SLOTS' && (
                    <div className="grid grid-cols-2 gap-2">
                      {uiState.data.map((slot: string, i: number) => (
                        <button key={i} onClick={() => sendTextMessage(slot)} className="bg-blue-50 hover:bg-blue-600 hover:text-white text-blue-700 border border-blue-200 text-xs font-bold py-2.5 rounded-xl transition">
                          {slot}
                        </button>
                      ))}
                    </div>
                  ...
                  {uiState.type === 'INPUT' && (
                    <form onSubmit={(e) => {
                      ...
                      {uiState.data.map((field: InputField, i: number) => (
                        <input key={i} required name={field.label} type={field.keyboard_type === 'emailAddress' ? 'email' : field.keyboard_type === 'phone' ? 'tel' : 'text'} placeholder={field.label} className="w-full text-sm p-2.5 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50" />
                      ))}
                      <button type="submit" className="w-full bg-blue-600 text-white text-sm font-bold py-2.5 rounded-lg hover:bg-blue-700 transition mt-1">Submit Details</button>
                    </form>
                  )}

                  {uiState.type === 'PAYMENT' && (
                    <div className="text-center">
                      <p className="text-xs text-slate-500 mb-3">Please pay the advance fee to confirm your slot.</p>
                      <button onClick={() => {
                        sendTextMessage("[SYSTEM_PAYMENT_SUCCESSFUL]", true);
                      }} className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-xl transition flex justify-center items-center gap-2 shadow-sm">
              ...
              <div className="p-3 bg-white border-t border-slate-100 flex items-center gap-2 relative z-0">
                <input 
                  ...
                  onChange={async (e) => {
                    ...
                    // FIX: Keep the entire Data URL string so Groq can parse it!
                    ...
                    wsRef.current?.send(JSON.stringify({
                      action: "INCOMING_CHAT",
                      message: "",
                      image: base64DataUrl, // Send the full string
                      mimeType: mimeType
                    }));
